...
export function ProjectCard({ project, index }: ProjectCardProps) {
	...
	const cardVariants = {
		...
		hover: {
			...
			rotateY: 5,
			boxShadow:
				"0 25px 50px -12px rgba(0, 0, 0, 0.25), 0 0 0 1px rgba(250, 204, 21, 0.1)",
			transition: {
	...
	return (
		<motion.div
			...
			<Card className="overflow-hidden border-yellow-500/20 bg-card/80 backdrop-blur-sm hover:border-yellow-500/40 transition-colors duration-300">
				...
				<div className="relative aspect-video overflow-hidden">
					<motion.div
						...
						<motion.div
							...
							className="absolute inset-0 bg-gradient-to-t from-yellow-500/20 via-transparent to-transparent flex items-center justify-center gap-2">
							<Button
								size="sm"
								variant="secondary"
					...
					<div className="absolute top-4 right-4">
						<motion.div
							className="w-8 h-8 rounded-full bg-yellow-500/90 text-black font-bold text-sm flex items-center justify-center backdrop-blur-sm"
							animate={
				...
				<motion.div
					className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-yellow-400 to-yellow-600 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
					initial={{ scaleX: 0 }}
